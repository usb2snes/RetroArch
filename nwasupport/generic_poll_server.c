...
static int64_t execute_command(generic_poll_server_client* client, char *cmd_str, char **args, int args_count)
{
    ...
    for (unsigned int i = 0; i < generic_emu_nwa_map_size; i++)
    {
        if ( generic_emu_nwa_map[i].command == command)
        {
...
bool  generic_poll_server_poll_code(int poll_timeout)
{
    static struct pollfd   poll_fds[6];
    static unsigned int    poll_fds_count = 1;
    ...
    poll_fds[0].events = POLLIN;
        ...
        if (ret < 0)
        {
            ...
            if (errno == EINTR) return false; // FIXME: or continue?
            return false;
        }
        // New clients ?
...
static bool generic_poll_server_start(int poll_timeout)
{
    ...
    while (true)
    {
        ...
        if (generic_poll_server_poll_code(poll_timeout) == false)
            break;
    }
    return true;
}
